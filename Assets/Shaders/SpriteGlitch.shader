        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                fixed4 frag(v2f i) : SV_Target {
                    ...
                    //Glitch control
                    if (frac(_Time.y) < 0.8) {
                        rightStripesFill = lerp(0, _RightStripesFill, frac(_Time.y) * 2);
                        leftStripesFill = lerp(0, _LeftStripesFill, frac(_Time.y) * 2);
                    }
                    if (frac(_Time.y) < 0.5) {
                        chromAberrAmount = lerp(fixed2(0, 0), _ChromAberrAmount.xy, frac(_Time.y) * 2);
                    }
                    if (frac(_Time.y) < 0.33) {
                        displAmount = lerp(fixed4(0,0,0,0), _DisplacementAmount, frac(_Time.y) * 3);
                    }
